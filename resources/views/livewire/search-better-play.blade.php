<div class="relative p-1 text-white bg-black border border-white">
    ...
    <input wire:model.debounce.500ms="searchBetterPlay" type="text" placeholder="Titles, people, genres" class="ml-8 text-white placeholder-gray-500 bg-black focus:outline-none">

    @if (strlen($searchBetterPlay >= 3))
    <div class="absolute w-64 p-2 mt-2 -ml-2 text-sm bg-gray-700">
        @if ($searchBetterPlayResults->count() > 0 )
        <ul>
            @foreach ($searchBetterPlayResults as $searchResults)
        ...
        @else
        <div class="px-3 py-3">
            <span> Your search for "{{ $searchBetterPlay }}" did not have any matches</span>
